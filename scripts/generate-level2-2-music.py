#!/usr/bin/env python3
"""Generate the five original Campfire Caldera Caper adaptive-music loops."""
...
SAMPLE_RATE = 32000
TAU = math.tau
RNG = np.random.default_rng(220035)
...
TRACKS = (
    Track("music_caldera_camp.ogg", "Taco Trek Trail", 102, 24, "camp"),
    Track("music_caldera_geyser.ogg", "Steam-Powered Picnic", 114, 24, "geyser"),
    Track("music_caldera_caves.ogg", "Lanterns Under Lava", 92, 20, "caves"),
    Track("music_caldera_eruption.ogg", "Caldera KABOOM Run", 134, 24, "eruption"),
    Track("music_caldera_luau.ogg", "Rainbow Lava Luau", 124, 24, "luau"),
)
...
def compose(track: Track) -> np.ndarray:
    ...
    # Each arrangement gets its own harmonic route and melodic silhouette.
    # This keeps the suite fresh while shared percussion colors and mix space
    # make the long equal-power crossfades feel like one continuous adventure.
    progressions = {
        "camp": (
            (60, (60, 64, 67)), (55, (55, 59, 62)),
            (57, (57, 60, 64)), (53, (53, 57, 60)),
        ),
        "geyser": (
            (64, (64, 68, 71)), (57, (57, 61, 64)),
            (61, (61, 64, 68)), (59, (59, 63, 66)),
        ),
        "caves": (
            (57, (57, 60, 64)), (53, (53, 57, 60)),
            (50, (50, 53, 57)), (52, (52, 56, 59)),
        ),
        "eruption": (
            (55, (55, 58, 62)), (51, (51, 55, 58)),
            (58, (58, 62, 65)), (53, (53, 57, 60)),
        ),
        "luau": (
            (62, (62, 66, 69)), (55, (55, 59, 62)),
            (57, (57, 61, 64)), (59, (59, 62, 66)),
        ),
    }
    motifs = {
        "camp": (60, 64, 67, 72, 69, 67, 64, 62),
        "geyser": (71, 68, 64, 66, 73, 71, 68, 64),
        "caves": (69, 72, 76, 74, 69, 67, 64, 68),
        "eruption": (55, 62, 67, 70, 67, 65, 62, 58),
        "luau": (69, 74, 78, 81, 78, 76, 73, 74),
    }
    progression = progressions[track.style]
    motif = motifs[track.style]

    if track.style in {"camp", "caves", "luau"}:
        add_water_bed(mix, 0.0018 if track.style == "caves" else 0.00105)

    for bar_index in range(track.bars):
        start = bar_index * bar
        root, chord = progression[bar_index % len(progression)]

        pad_level = {"camp": 0.024, "geyser": 0.012, "caves": 0.045, "eruption": 0.009, "luau": 0.028}[track.style]
        for note_index, note in enumerate(chord):
            add_instrument(mix, start, bar * 0.98, note, pad_level, "pad", (note_index - 1) * 0.42)

        bass_level = {"camp": 0.075, "geyser": 0.08, "caves": 0.052, "eruption": 0.105, "luau": 0.095}[track.style]
        ...
        if track.style in {"camp", "geyser", "luau"}:
            chord_instrument = "ukulele" if track.style != "geyser" else "marimba"
            chord_level = 0.082 if track.style == "luau" else 0.066
            for beat_index in range(4):
                for note_index, note in enumerate(chord):
                    add_instrument(
                        mix,
        ...
        if bar_index % 2 == 0:
            ...
            melody_level = {"camp": 0.105, "geyser": 0.095, "caves": 0.082, "eruption": 0.09, "luau": 0.115}[track.style]
            for note_index, note in enumerate(motif):
                ...
                add_instrument(
                    mix,
                    note_start,
                    beat * (0.72 if track.style != "caves" else 1.25),
                    note + variation,
                    melody_level,
                    melody_instrument,
                    -0.35 + (note_index / 7.0) * 0.7,
                )
                if track.style == "luau" and note_index % 2 == 0:
                    add_instrument(mix, note_start + beat * 0.25, beat * 0.5, note - 12, 0.045, "marimba", 0.45)

        if track.style == "caves" and bar_index % 4 == 3:
            for note_index, note in enumerate((74, 73, 69, 66)):
                add_instrument(mix, start + note_index * beat, beat * 1.55, note, 0.05, "flute", 0.34)

        if track.style in {"eruption", "luau"}:
            add_kick(mix, start, 0.105 if track.style == "eruption" else 0.09)
            add_kick(mix, start + beat * 2, 0.085)

        shaker_steps = 8 if track.style in {"camp", "geyser", "eruption", "luau"} else 4
        shaker_level = {"camp": 0.018, "geyser": 0.024, "caves": 0.01, "eruption": 0.029, "luau": 0.027}[track.style]
        for step in range(shaker_steps):
            add_shaker(mix, start + step * bar / shaker_steps, (-1) ** step * 0.62, shaker_level)

        conga_level = {"camp": 0.045, "geyser": 0.056, "caves": 0.022, "eruption": 0.073, "luau": 0.068}[track.style]
        add_conga(mix, start + beat, True, -0.38, conga_level)
        add_conga(mix, start + beat * 2.5, False, 0.38, conga_level)
        if track.style in {"geyser", "eruption", "luau"}:
            add_conga(mix, start + beat * 3.5, True, 0.2, conga_level * 0.8)

        add_clave(mix, start + beat * 1.5, -0.48, 0.026 if track.style == "caves" else 0.035)
        add_clave(mix, start + beat * 3.5, 0.48, 0.026 if track.style == "caves" else 0.035)
...
def main() -> None:
    ...
    output_dir = root / "public" / "game" / "assets" / "level2_2_music"
    output_dir.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="jft-level2-2-music-") as temp_dir:
