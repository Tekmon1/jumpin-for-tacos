(() => {
  const SOURCE_VERSION = 'w1-2-v33-super-taco-hero';
  ...
  function drawPlayer(time) {
    ...
    const x = player.x - game.cameraX + player.w / 2; const y = player.y + player.h / 2; sharedAbilities.drawHeroEffects(ctx, game.abilities, player, game.cameraX, time); ctx.save(); if (player.invulnerable > 0 && Math.floor(player.invulnerable * 12) % 2 === 0) ctx.globalAlpha = .42;
    ...
    sharedAbilities.applyHeroStyle(ctx, game.abilities); const sourceW = images.hero.width / 8; ctx.drawImage(images.hero, frame * sourceW, 0, sourceW, images.hero.height, -33, -33, 66, 66); ctx.restore();
  }
